...
window.addEventListener('load', () => {
	...
	var scripts = []
	var script_src
	document.querySelectorAll("script[type='text/perl']").forEach((el) => {
		if (el.src) {
...
Perl.addStateChangeListener = (handler) => {
...
Perl.Util.baseurl = (urlstr) => {
	var url = new URL(urlstr)
	if (url.protocol === 'file:') return url.href.substring(0, url.href.lastIndexOf('/'))
	else return url.origin + url.pathname.substring(0, url.pathname.lastIndexOf('/'))
}

Perl.init = (readyCallback) => {
	if (Perl.state !== 'Uninitialized') throw `Perl: can't call init in state ${Perl.state}`
	...
	Module = {
		...
		onExit: (status) => {
			...
			if (status === 0) console.debug('Perl: Exit status', status, '(state', Perl.state, ')')
			else {
...
Perl.start = (argv) => {
	if (Perl.state !== 'Ready') throw `Perl: can't call start in state ${Perl.state}`
...
Perl.eval = (code) => {
	if (Perl.state !== 'Running') throw `Perl: can't call eval in state ${Perl.state}`
...
Perl.glue = (ref) => {
	...
	if (free_ids.length > 0) {
		...
		if (Perl.next_glue_id >= Number.MAX_SAFE_INTEGER) throw 'Perl.GlueTable is overflowing!'
