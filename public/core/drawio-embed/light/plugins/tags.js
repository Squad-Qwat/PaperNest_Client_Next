...
Draw.loadPlugin((editorUi) => {
	...
	// Adds action
	editorUi.actions.addAction('hiddenTags...', () => {
		if (editorUi.hiddenTagsWindow == null) {
			editorUi.hiddenTagsWindow = new HiddenTagsWindow(
				editorUi,
			...
			editorUi.hiddenTagsWindow.window.addListener('show', () => {
				editorUi.fireEvent(new mxEventObject('hiddenTags'))
			})
			editorUi.hiddenTagsWindow.window.addListener('hide', () => {
				editorUi.fireEvent(new mxEventObject('hiddenTags'))
			})
	...
	var HiddenTagsWindow = function (editorUi, x, y, w, _h) {
		var _graph = editorUi.editor.graph
		...
		function _setCellsVisibleForTag(tag, visible) {
		...
		function updateSelectedTags(tags, selected, selectedColor, filter) {
			...
			mxUtils.write(
				title,
				filter != null ? 'Select hidden tags:' : 'Or add/remove existing tags for cell(s):'
			)
		...
		function refreshUi() {
			if (graph.isSelectionEmpty()) {
				...
				updateSelectedTags(
					getAllTags(),
					getLookup(graph.getCommonTagsForCells(graph.getSelectionCells())),
					'#2873e1'
				)
		...
		graph.selectionModel.addListener(mxEvent.CHANGE, (_sender, _evt) => {
			refreshUi()
		})

		graph.model.addListener(mxEvent.CHANGE, (_sender, _evt) => {
			refreshUi()
		})

		mxEvent.addListener(filterInput, 'keyup', () => {
			updateTagCloud(getAllTags())
		})

		mxEvent.addListener(searchInput, 'keyup', (evt) => {
			// Ctrl or Cmd keys
			if (evt.keyCode === 13) {
