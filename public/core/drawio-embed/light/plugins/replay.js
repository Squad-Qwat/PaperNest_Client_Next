...
Draw.loadPlugin((ui) => {
	...
	function decodeChanges(delta, direct) {
		...
		codec2.lookup = (id) => model.getCell(id)
	...
	if (ui.editor.isChromelessView()) {
		...
		var delay = parseInt(urlParams['delay-delay'] || 1000, 10)
		...
		codec.lookup = (id) => model.getCell(id)

		model.addListener(mxEvent.CHANGE, (_sender, evt) => {
			if (tape != null) {
					...
					(newValue) => {
						if (newValue != null) {
							var dlg = new EmbedDialog(
								ui,
								'https://app.diagrams.net/?p=replay&lightbox=1&replay-delay=' +
									parseFloat(newValue) +
									'&replay-data=' +
		...
		ui.actions.addAction('replay...', () => {
			var dlg = new TextareaDialog(
				ui,
				'Changes [JSON export, compressed edits or <edit>..</edit>]:',
				'',
				(newValue) => {
					if (newValue.length > 0) {
						try {
							...
							if (ui.buttonContainer != null) {
								...
								var button = mxUtils.button('Step', () => {
									if (!step()) {
