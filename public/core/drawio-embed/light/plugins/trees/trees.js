...
Draw.loadPlugin((ui) => {
	...
	ui.actions.addAction(
		...
		() => {
			if (graph.isEnabled() && graph.getSelectionCount() === 1) {
				...
				graph.traverse(cell, true, (vertex, edge) => {
					if (edge != null) {
						subtree.push(edge)
					}
	...
	graph.foldCells = function (collapse, _recurse, cells, _checkFoldable, _evt) {
		...
		try {
			...
			for (var i = 0; i < cells.length; i++) {
				if (isTreeCell(cells[i])) {
					graph.traverse(cells[i], true, (vertex, edge) => {
						if (edge != null) {
							tmp.push(edge)
						}
	...
	graph.removeCells = function (cells, _includeEdges) {
		var tmp = []
					...
					if (isTreeCell(cells[i])) {
						...
						if (newEdges.length === 0 && edges.length > 0) {
							...
							this.addEdge(
								...
								this.model.getTerminal(edges[0], true),
								result[i]
							)
						}
					}
	...
	graph.moveCells = function (cells, dx, dy, _clone, target, _evt, _mapping) {
		var result = null
		...
		try {
			...
			if (isTreeCell(target)) {
				...
				for (var i = 0; i < cells.length; i++) {
					if (
						isTreeCell(cells[i]) ||
						(graph.model.isEdge(cells[i]) && graph.model.getTerminal(cells[i], true) == null)
					) {
						target = null
						break
					}
			...
			if (result.length === cells.length) {
				for (var i = 0; i < result.length; i++) {
					...
					if (this.model.isEdge(result[i])) {
						if (
							isTreeCell(newSource) &&
							mxUtils.indexOf(result, this.model.getTerminal(result[i], true)) < 0
						) {
						...
						if (edges.length > 0) {
							if (!clone) {
								if (
									isTreeCell(newSource) &&
									mxUtils.indexOf(cells, this.model.getTerminal(edges[0], true)) < 0
								) {
	...
	ui.sidebar.dropAndConnect = function (source, _targets, _direction, _dropCellIndex) {
					...
					if (abs != null && abs.length > 0) {
						...
						if (pt != null) {
							if (pt.y === state.y && Math.abs(pt.x - state.getCenterX()) < state.width / 2) {
								...
								Math.abs(pt.x - state.getCenterX()) < state.width / 2
							) {
	...
	function addSibling(cell, after) {
		...
		try {
			...
			if (dir === mxConstants.DIRECTION_SOUTH || dir === mxConstants.DIRECTION_NORTH) {
				clones[1].geometry.x += after
					? cell.geometry.width + spacing
					: -clones[1].geometry.width - spacing
			} else {
				clones[1].geometry.y += after
					? cell.geometry.height + spacing
			...
			if (state != null) {
				...
				if (dir === mxConstants.DIRECTION_SOUTH || dir === mxConstants.DIRECTION_NORTH) {
					bbox.x +=
						(after ? cell.geometry.width + spacing : -clones[1].geometry.width - spacing) * s
				} else {
					bbox.y +=
						(after ? cell.geometry.height + spacing : -clones[1].geometry.height - spacing) * s
				}
					...
					for (var i = 0; i < sib.length; i++) {
						...
						if (dir === getTreeDirection(temp)) {
							...
							if (temp !== cell && sibling != null) {
								if (
									(hor && after !== sibling.getCenterX() < state.getCenterX()) ||
									...
									if (mxUtils.intersects(bbox, sibling)) {
										dx =
											...
											Math.max(
												dx,
												(Math.min(bbox.x + bbox.width, sibling.x + sibling.width) -
										...
										dy =
											...
											Math.max(
												dy,
												(Math.min(bbox.y + bbox.height, sibling.y + sibling.height) -
						...
						if (dir === getTreeDirection(temp)) {
							...
							if (temp !== cell && sibling != null) {
								if (
									...
									graph.traverse(sibling.cell, true, (vertex, edge) => {
										if (edge != null) {
											subtree.push(edge)
										}
	...
	function addChild(cell) {
		graph.model.beginUpdate()
		try {
			var _edges = graph.getIncomingEdges(cell)
			var clones = graph.cloneCells([edges[0], cell])
			...
			if (dir === mxConstants.DIRECTION_SOUTH) {
				clones[1].geometry.x =
					bbox == null
						? cell.geometry.x + (cell.geometry.width - clones[1].geometry.width) / 2
				...
				clones[1].geometry.y =
					bbox == null
						? cell.geometry.y + (cell.geometry.height - clones[1].geometry.height) / 2
		...
		if (state != null && sib != null) {
			for (var i = 0; i < sib.length; i++) {
				...
				if (
					temp != null &&
					((!horizontal &&
						Math.min(temp.x + temp.width, state.x + state.width) >= Math.max(temp.x, state.x)) ||
						(horizontal &&
							Math.min(temp.y + temp.height, state.y + state.height) >= Math.max(temp.y, state.y)))
				) {
					targets.push(temp)
				}
			}
			...
			targets.sort((a, b) =>
				horizontal ? a.x + a.width - b.x - b.width : a.y + a.height - b.y - b.height
			)
		}
		...
		if (h1 === h2 && dir !== direction) {
			...
			if (edges != null && edges.length > 0) {
				...
				if (state != null) {
					...
					if (idx >= 0) {
						idx +=
							direction === mxConstants.DIRECTION_NORTH || direction === mxConstants.DIRECTION_WEST
								? -1
								: 1
	...
	graph.connectVertex = function (source, direction, length, evt, forceClone, ignoreCellAt) {
		if (isTreeCell(source) && source.getAttribute('treeRoot') !== '1') {
			...
			this.model.beginUpdate()
			try {
				var cells = graphConnectVertex.call(
					this,
					source,
					direction,
					length,
	...
	graph.graphHandler.getCells = function (_initialCell) {
		...
		for (var i = 0; i < temp.length; i++) {
			if (isTreeCell(temp[i])) {
				...
				graph.traverse(temp[i], true, (vertex, edge) => {
	...
	function mxIconSet(state) {
		...
		mxEvent.addGestureListeners(
			...
			mxUtils.bind(this, function (evt) {
				...
				graph.graphHandler.bounds = graph.graphHandler.graph
					.getView()
					.getBounds(graph.graphHandler.cells)
	...
	graph.addMouseListener({
		...
		mouseMove: function (_sender, me) {
			...
			if (
				(graph.isMouseDown && !mxEvent.isTouchEvent(me.getEvent())) ||
				graph.isEditing() ||
		...
		mouseUp: (_sender, _me) => {},
		dragEnter: function (_evt, state) {
			if (this.currentIconSet == null) {
				this.currentIconSet = new mxIconSet(state)
			}
		},
		dragLeave: function (_evt, _state) {
			if (this.currentIconSet != null) {
	...
	function addPalette() {
		sb.addPalette('trees', 'Trees', false, (content) => {
			;(() => {
				var cell = new mxCell(
					'Central Idea',
					new mxGeometry(0, 20, 100, 40),
				...
				content.appendChild(
					sb.createVertexTemplateFromCells([edge, edge2, cell, cell2, cell3], 240, 66, 'Mindmap')
				)
			})()
			...
			;(() => {
				...
				content.appendChild(
					sb.createVertexTemplateFromCells([edge, edge2, cell, cell2, cell3], 240, 160, 'Orgchart')
				)
			})()
			...
			;(() => {
				...
				content.appendChild(
					sb.createVertexTemplateFromCells([edge, edge2, cell, cell2], 220, 60, 'Sub Sections')
				)
			})()
		})
	}
