...
Draw.loadPlugin((ui) => {
	...
	graph.view.validateCellState = function (_cell, recurse) {
	...
	graph.cellRenderer.getShapesForState = (state) => [
		state.shape,
		state.text,
		state.secondLabel,
		state.control,
	...
	// Adds action
	var action = ui.actions.addAction('number...', () => {
		enabled = !enabled
		graph.refresh()
	})

	action.setToggleAction(true)
	action.setSelectedCallback(() => enabled)
