...
mxWebColaAdaptor.prototype.defaultValues = {
	...
	nodeSpacing: function nodeSpacing(_node) {
		return 10
...
mxWebColaAdaptor.prototype.updatePositions = (_isUndoable) =>
	/**
...
mxWebColaAdaptor.prototype.step = function (_colaAdaptor) /**
...
mxWebColaAdaptor.prototype.finish = () => {}
 ...
 */
{
	...
	var colaAdaptor = (this.adaptor = cola.adaptor({
		trigger: (evt) => {
	...
	this.callback = () => {
		this.renderingChain(colaAdaptor)
	}
...
function _getScreenConstraints(layout, width, height) /**
 ...
 */
{
	...
	for (var id in cells) {
		...
		var _bounds = view.getBoundingBox(state, true)
	...
	for (var id in cells) {
		...
		if (cell.isEdge() && cell.getTerminal(true) != null && cell.getTerminal(false) != null) {
			...
			if (this.isGroup(terminal1) || this.isGroup(terminal2)) {
				addedLinks = this.addGroupConstraintLinks(
					terminal1,
					terminal2,
					activeVertices,
					inactiveToActiveMap,
	...
	for (var i = 0; i < preliminaryGroups.length; i++) {
		...
		if (Object.hasOwn(graphGroup, 'leaves') || Object.hasOwn(graphGroup, 'groups')) {
...
mxWebColaAdaptor.prototype.createLink = (sourceId, targetId, cellIds) =>
	/**
...
mxWebColaAdaptor.prototype.getActiveVerticesInGroup = function (
	groupCell,
	activeVertices,
	includeCollapsedGroups
) /**
...
mxWebColaAdaptor.prototype.hasVertexChildren = (_cell) =>
	/**
...
mxWebColaAdaptor.prototype.isInCollapsedTree = (cell) => {
...
mxWebColaAdaptor.prototype.isGroup = (cell) => cell.children != null && cell.children.length > 0

mxWebColaAdaptor.prototype.addGroupConstraintLinks = function (
	groupA,
	groupB,
	activeVertices,
	inactiveToActiveMap,
...
mxWebColaAdaptor.prototype.getUniqueLinks = (links) =>
	/**
...
if (
	...
	(true ? 'undefined' : typeof undefined)
) {
	doRendering =
	...
	function _doRendering(callback) {
		callback()
	}
}
