...
Draw.loadPlugin((editorUi) => {
	if (editorUi.editor.isChromelessView()) {
		...
		var updateInterval = parseInt(urlParams['update-interval'] || 60000, 10)
		...
		function scheduleUpdates() {
			...
			if (urlParams['update-url'] || (root.value != null && typeof root.value === 'object')) {
				if (root.value != null && typeof root.value === 'object') {
					updateInterval = parseInt(root.value.getAttribute('updateInterval') || updateInterval, 10)
				...
				if (updateUrl != null) {
					...
					function doUpdate() {
						if (updateUrl === 'demo') {
							...
							mxUtils.post(
								updateUrl,
								`xml=${encodeURIComponent(currentXml)}`,
								(req) => {
			...
			for (var id in graph.model.cells) {
				...
				if (graph.model.isEdge(cell)) {
					...
					if (Math.random() > 0.5 && state != null && state.length > 50) {
						...
						update.setAttribute(
							'value',
							'<object label="%load% minutes" load="' +
								Math.round(Math.random() * 100) +
								'" placeholders="1">'
						)
						update.setAttribute(
							'style',
							cell.style + ';strokeColor=red;strokeWidth=' + Math.round(Math.random() * 5) + ';'
						)
						...
						update.setAttribute(
							'value',
							'<object label="" load="' + Math.round(Math.random() * 100) + '" placeholders="1">'
						)
					...
					if (
						...
						if (Math.random() > 0.5) {
							var _update = doc.createElement('update')
							update.setAttribute('id', cell.id)
							update.setAttribute(
								'value',
								'<object tooltip="%load%% Done" load="' +
									Math.round(Math.random() * 100) +
									'" placeholders="1">'
							)
							...
							update.setAttribute(
								...
								JSON.stringify({
									...
									valign: 'top',
									image: {
										src: 'https://app.diagrams.net/mxgraph/images/warning.gif',
										width: 26,
										height: 26,
									},
								})
							...
							update.setAttribute(
								'style',
								`${cell.prevStyle};fillColor=#d4e1f5;gradientColor=white;`
							)
							update.setAttribute(
								'value',
