...
Draw.loadPlugin((ui) => {
	...
	function AddRow(propertyModel, tableName) {
		...
		if (
			...
			propertyModel.ForeignKey !== undefined &&
			propertyModel.ForeignKey !== null
		) {
			propertyModel.ForeignKey.forEach((foreignKeyModel) => {
				//We do not want the foreign key to be duplicated in our table to the same property
				if (
					tableName !== foreignKeyModel.PrimaryKeyTableName ||
					(tableName === foreignKeyModel.PrimaryKeyTableName &&
						propertyModel.Name !== foreignKeyModel.PrimaryKeyName)
				) {
	...
	function ParseMySQLForeignKey(name, currentTableModel) {
		...
		//Create ForeignKey
		var foreignKeyOriginModel = CreateForeignKey(
			foreignKey,
		...
		//Create ForeignKey
		var foreignKeyDestinationModel = CreateForeignKey(
			referencedPropertyName,
			referencedTableName,
			foreignKey,
			currentTableModel.Name,
	...
	function ParseSQLServerForeignKey(name, _currentTableModel) {
		var referencesIndex = name.toLowerCase().indexOf('references')

		if (name.toLowerCase().indexOf('foreign key(') !== -1) {
			var _foreignKeySQL = name
	...
	var _btn = mxUtils.button('Insert MySQL', () => {
		try {
	...
	var btn = mxUtils.button('Insert SQL Server', () => {
		try {
