...
Draw.loadPlugin(function (ui) {
		...
		if (ncUser == null) {
			ui.remoteInvoke(
				...
				(user) => {
					ncUser =
						user == null
							? new DrawioUser(Date.now(), null, 'Anonymous')
	...
	ui.isRevisionHistoryEnabled = () => {
		var file = ui.getCurrentFile()
		return file?.desc && (!file.desc.ver || file.desc.versionsEnabled)
	}

	ui.isRevisionHistorySupported = () => ui.isRevisionHistoryEnabled()

	/**
	 * Get revisions of current file
	 */
	ui.getRevisions = (success, error) => {
	 ...
	 *
	 */
	EmbedFile.prototype.isConflict = (err) => err != null && err.status === 409

	/**
	 * Returns the current user.
	 */
	EmbedFile.prototype.getCurrentUser = () => ui.getCurrentUser()

	EmbedFile.prototype.isRealtimeSupported = () => true
	 ...
	 *
	 */
	EmbedFile.prototype.saveFile = function (
		_title,
		_revision,
		success,
		error,
				...
				var doSave = mxUtils.bind(this, function () {
					try {
						...
						ui.remoteInvoke(
							'saveFile',
							this.desc.ver > 1
								? [this.desc.id, this.desc.shareToken, savedData, etag]
	 ...
	 */
	EmbedFile.prototype.isAutosaveOptional = () => true
	 ...
	 *
	 */
	EmbedFile.prototype.isRevisionHistorySupported = () => true

	EmbedFile.prototype.isOptimisticSync = () => true
	EmbedFile.prototype.getSize = function () {
		return this.desc.size
	}

	EmbedFile.prototype.isEditable = function () {
		return this.desc?.writeable
	}
	...
	EmbedFile.prototype.getLatestVersion = function (success, error) {
		ui.remoteInvoke(
			'loadFile',
			this.desc.ver > 1 ? [this.desc.id, this.desc.shareToken] : [this.desc.path],
			null,
			mxUtils.bind(this, (data) => {
				var xml = data.xml
				delete data.xml
	 ...
	 *
	 */
	EmbedFile.prototype.getDescriptorEtag = (desc) => desc.etag

	/**
	 *
	 */
	EmbedFile.prototype.setDescriptorEtag = (desc, etag) => {
		desc.etag = etag
	}
	...
	EmbedFile.prototype.isAutosaveNow = (_success, _error) => allowAutoSave

	//Ensure saving is via the file
	ui.actions.get('save').funct = (exit) => {
	...
	ui.installMessageHandler = function (callback) {
		origInstallMessageHandler.call(this, function (_xml, evt) {
			try {
				// New empty files
