...
Draw.loadPlugin((ui) => {
	...
	// Adds action
	ui.actions.addAction('randomLabel', () => {
		var cells = graph.getSelectionCells().slice()

		if (cells.length > 0) {
			var delay = parseInt(prompt('Delay (ms)', defaultDelay), 10)

			if (!Number.isNaN(delay)) {
				defaultDelay = delay
				var max = parseInt(prompt('Cycles', defaultMax), 10)
	...
	// Adds action
	ui.actions.addAction('swapChildren', () => {
		var cells = graph.getSelectionCells().slice()
						...
						window.setTimeout(() => {
							...
							if (childrenA != null && childrenB != null) {
								...
								try {
									...
									console.log(
										`random.js: ${counter} - swapping ${passes} children from parent A to parent B`
									)
									...
									console.log(
										`random.js: ${counter} - swapping ${passes} children from parent B to parent A`
									)
	...
	// Adds action
	ui.actions.addAction('placeChildren', () => {
		var graph = ui.editor.graph
		var cells = graph.getSelectionCells().slice()
		...
		if (cells.length > 1) {
			...
			function schedule() {
				...
				window.setTimeout(() => {
					...
					if (
						(childrenA != null && childrenA.length > 1) ||
						(childrenB != null && childrenB.length > 1)
					) {
						graph.getModel().beginUpdate()
						try {
	...
	// Adds action
	ui.actions.addAction('reorderChildren', () => {
		var cells = graph.getSelectionCells().slice()
	...
	menu.funct = function (menu, parent) {
		...
		ui.menus.addMenuItems(
			menu,
			['-', 'randomLabel', 'reorderChildren', 'swapChildren', 'placeChildren'],
			parent
		)
	}
})
