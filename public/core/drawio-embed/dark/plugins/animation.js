...
Draw.loadPlugin((editorUi) => {
	...
	var AnimationWindow = function (editorUi, x, y, w, h) {
		...
		var runBtn = mxUtils.button('Preview', () => {
		...
		var stopBtn = mxUtils.button('Stop', () => {
			graph.getModel().clear()
			stop()
		})
		td21.appendChild(stopBtn)

		var applyBtn = mxUtils.button('Apply', () => {
			var root = editorUi.editor.graph.getModel().getRoot()
		...
		editorUi.editor.graph.addListener(mxEvent.ROOT, () => {
