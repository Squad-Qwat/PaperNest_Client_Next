...
const TableHeaderSection = React.memo(({ table }: { table: any }) => {
	return (
...
export const CitationTable = React.memo(({ 
	...
	const columns = useMemo<ColumnDef<CitationDisplay>[]>(() => [
		{
			id: 'select',
			header: ({ table }) => (
				<Checkbox
	...
	const renderContent = () => {
		if (isLoading) {
			return ['skel-1', 'skel-2', 'skel-3', 'skel-4', 'skel-5'].map((rowId) => (
				<TableRow key={rowId}>
					{columns.map((column, colIndex) => {
						const columnId = column.id || (column as any).accessorKey || colIndex
						return (
