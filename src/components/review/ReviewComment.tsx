import { Quote } from 'lucide-react'
import { Badge } from '@/components/ui/badge'
import { Card } from '@/components/ui/card'

interface ReviewCommentProps {
	authorName: string
	date: string
	content: string
	userType: 'lecturer' | 'student' | 'system'
}

export function ReviewComment({
	authorName,
	date,
	content,
	userType,
}: Readonly<ReviewCommentProps>) {
	const isLecturer = userType === 'lecturer'
	const isSystem = userType === 'system'

	if (isSystem) {
		return (
			<div className='flex justify-center my-2'>
				<Badge
					variant='outline'
					className='px-3 py-0.5 rounded-full text-[9px] font-bold text-muted-foreground uppercase tracking-widest bg-muted/50 border-border'
				>
					{content} • {date}
				</Badge>
			</div>
		)
	}

	return (
		<div className='flex flex-col gap-1.5 group relative w-full'>
			<div className='flex items-center justify-between px-1'>
				<div className='flex items-center gap-2'>
					<div
						className={`w-1.5 h-1.5 rounded-full ${isLecturer ? 'bg-amber-500' : 'bg-teal-500'}`}
					/>
					<span className='font-bold text-foreground text-[13px]'>{authorName}</span>
					<Badge
						variant='secondary'
						className={`text-[8px] font-bold px-1.5 py-0 rounded-full uppercase tracking-tighter border-none h-4 flex items-center ${
							isLecturer
								? 'bg-amber-100 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300'
								: 'bg-teal-100/60 dark:bg-teal-950/40 text-teal-700 dark:text-teal-300'
						}`}
					>
						{userType}
					</Badge>
				</div>
				<span className='text-[10px] text-muted-foreground font-medium'>{date}</span>
			</div>

			<Card
				className={`p-4 rounded-lg shadow-none border-none ring-1 leading-relaxed text-sm relative overflow-hidden ${
					isLecturer
						? 'bg-amber-50/50 dark:bg-amber-950/20 ring-amber-100 dark:ring-amber-900/50 text-amber-900 dark:text-amber-200'
						: 'bg-card ring-border text-foreground'
				}`}
			>
				{isLecturer && (
					<Quote className='w-8 h-8 absolute -left-1 -top-1 text-amber-200/20 dark:text-amber-700/20 transform rotate-180 pointer-events-none' />
				)}
				<div className={`relative z-10 ${isLecturer ? 'italic pl-2 font-medium' : ''}`}>
					{content}
				</div>
			</Card>
		</div>
	)
}
