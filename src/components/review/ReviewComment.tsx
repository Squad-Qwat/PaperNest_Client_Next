	...
	if (isSystem) {
		return (
			<div className='flex justify-center my-2'>
				<Badge variant="outline" className='px-3 py-0.5 rounded-full text-[9px] font-bold text-gray-400 uppercase tracking-widest bg-gray-50 border-gray-100'>
					{content} • {date}
				</Badge>
			</div>
		)
		...
		<div className='flex flex-col gap-1.5 group relative w-full'>
			<div className='flex items-center justify-between px-1'>
				<div className='flex items-center gap-2'>
					<div className={`w-1.5 h-1.5 rounded-full ${isLecturer ? 'bg-amber-500' : 'bg-teal-500'}`} />
					<span className='font-bold text-gray-900 text-[13px]'>{authorName}</span>
					<Badge variant="secondary" className={`text-[8px] font-bold px-1.5 py-0 rounded-full uppercase tracking-tighter border-none h-4 flex items-center ${
			...
			<Card className={`p-4 rounded-lg shadow-none border-none ring-1 leading-relaxed text-sm relative overflow-hidden ${
				isLecturer 
