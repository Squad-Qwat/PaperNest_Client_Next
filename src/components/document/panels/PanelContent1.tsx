		...
		try {
			...
			await addDocumentFile.mutateAsync({
				...
				file: {
					...
					r2Key: key,
					size: file.size,
					createdAt: new Date() as unknown as Date, // bypass strict typing for now if needed
				},
			})
		...
		<div className='flex flex-col h-full bg-white'>
				...
				{isLoading ? (
					...
					<div className='flex flex-col items-center justify-center p-8 text-center gap-3 opacity-40'>
						...
						<div>
							...
							<p className='text-xs text-gray-400 mt-1 uppercase tracking-tighter'>
								Upload images, .bib, or other source files
							</p>
						</div>
					</div>
				) : (
					<div className='divide-y divide-gray-50'>
						{files.map((file) => (
							<div
								key={file.fileId}
								className='group flex items-center gap-3 p-3 hover:bg-gray-50/80 transition-colors'
							>
			...
			<div className='p-3 bg-blue-50/30 border-t border-blue-50'>
				<p className='text-[10px] text-blue-600 font-medium leading-relaxed italic'>
					Tip: Files uploaded here can be used in your LaTeX document. For example, use
					\includegraphics{'{filename}'} for images.
				</p>
			</div>
		</div>
	)
