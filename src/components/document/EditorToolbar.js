...
const EditorToolbar = ({ 
  ...
  const applyTextStyle = async (style, action) => {
    ...
    try {
      ...
      if (action === 'apply') {      // Select the entire paragraph
      editor.chain()
        .setTextSelection({ from: paragraphStart, to: paragraphEnd })
        .run()
      ...
      if (styleDef) {
        ...
        const fontFamilyValue = fontFamilies.find(f => f.name === currentFontFamily)?.value || '"Times New Roman", Times, serif'
  ...
  const updateAllInstancesOfStyle = async (styleName, styleDef) => {
    ...
    try {
      ...
      console.log(`Found ${positions.length} instances of ${styleName} (including list content for Normal Text)`)
  ...
  const getStyleFontSize = (styleName) => {
    try {
      ...
      switch (styleName) {
        case 'Heading 1': return '24pt';
        case 'Heading 2': return '18pt';
        case 'Heading 3': return '14pt';
        case 'Heading 4': return '12pt';
        default: return '11pt';
      }
    } catch (error) {
    ...
    <div className={`bg-white border-t border-gray-200 px-11 py-1 sticky top-[4.5rem] z-30 transition-all duration-300 ${aiAssistantOpen ? 'mr-80' : 'mr-0'}`}>
      <div className="flex items-center gap-1 overflow-x-auto pb-1 scrollbar-hide min-h-[40px]">
        ...
        <div className="relative" ref={textStyleRef}>
          ...
          {textStyleOpen && mounted && createPortal(
            <>
                    ...
                    {hoveredStyle === style.name && (
                      <div className="absolute left-full top-0 bg-white border border-gray-200 rounded-md shadow-md min-w-[200px] z-[10000]">
                        ...
                        >
                          ...
                          <div className="text-gray-500 text-[10px]">Change current paragraph to this style</div>
                        </button>
                        <button
                          onClick={() => {
                            if (!editor || editor.isDestroyed) return
                        ...
                        >
                          <div className="font-medium">Update &quot;{style.name}&quot; to match</div>
                          <div className="text-gray-500 text-[10px]">Update all instances in document</div>
                        </button>
                      </div>
                    )}
                  </div>
        ...
        <div className="relative" ref={fontFamilyRef}>
          ...
          {fontFamilyOpen && mounted && createPortal(
            <>
              <div className="fixed inset-0 z-[9998]" onClick={() => setFontFamilyOpen(false)}></div>
              <div 
                ...
                {fontFamilies.map((font) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-xs hover:bg-gray-100 first:rounded-t-md last:rounded-b-md ${
                      editor?.isActive('textStyle', { fontFamily: font.value }) ? 'bg-blue-50 text-blue-600' : ''
                    }`}
        ...
        <div className="flex items-center border border-gray-200 rounded-md">
          ...
          <div className="px-2 text-xs min-w-[30px] text-center">
            {getCurrentFontSize()}pt
          </div>
          <Button
            variant="ghost"
            size="icon"
        ...
        <div className="relative" ref={lineSpacingRef}>
          ...
          {lineSpacingOpen && mounted && createPortal(
            <>
              <div className="fixed inset-0 z-[9998]" onClick={() => setLineSpacingOpen(false)}></div>
              <div 
        ...
        <div className="flex items-center border border-gray-200 rounded-md">
          <Button
            ...
            className={`h-7 w-7 rounded-r-none border-r border-gray-200 ${
              editor?.isActive({ textAlign: 'left' }) || (!editor?.isActive({ textAlign: 'center' }) && !editor?.isActive({ textAlign: 'right' }) && !editor?.isActive({ textAlign: 'justify' })) ? 'bg-gray-200' : ''
            }`}
            disabled={!editor}
            title="Align Left"
          >
