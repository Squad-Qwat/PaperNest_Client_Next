...
export const LaTeXLayoutCommand = TiptapNode.create({
	...
	renderHTML({ HTMLAttributes }) {
		...
		const style = 'color: var(--primary, #3b82f6); font-family: var(--font-mono); font-size: 0.8em;'
	...
	renderHTML({ HTMLAttributes, node }) {
		return [
			...
			mergeAttributes(HTMLAttributes, {
				...
				style:
					'background: #f3f4f6; padding: 1rem; border-radius: 0.5rem; border: 1px dashed #d1d5db;',
			}),
	...
	renderHTML({ HTMLAttributes }) {
		...
		return [
			...
			mergeAttributes(HTMLAttributes, {
				...
				style:
					'background: #f5f5f5; border: 1px dashed #ccc; border-radius: 4px; padding: 8px 12px; margin: 8px 0; font-family: monospace; font-size: 12px; color: #666; cursor: not-allowed; user-select: none;',
			}),
			`[${blockType}]`,
		]
	},
	...
	renderHTML({ HTMLAttributes }) {
		...
		const label =
			cmd !== 'cmd'
				? `\\${cmd}{...}`
				: rawLatex
					? `${rawLatex.slice(0, 36)}${rawLatex.length > 36 ? '…' : ''}`
					: '\\cmd{...}'
		// NOTE: data-latex is already escaped by the attribute's renderHTML
		return [
			'span',
			mergeAttributes(HTMLAttributes, {
				...
				style:
					'background: #e3f2fd; border: 1px solid #90caf9; border-radius: 3px; padding: 0 4px; font-family: monospace; font-size: 0.9em; color: #1565c0; cursor: not-allowed; user-select: none;',
			}),
			label,
		]
	},
...
export const LaTeXNewline = TiptapNode.create({
	...
	addAttributes() {
		return {
			count: {
				default: 1,
				parseHTML: (element) => parseInt(element.getAttribute('data-count') || '1', 10),
				renderHTML: (attributes) => {
		...
		return [
			{
				...
				getAttrs: (element) => ({
					count: parseInt((element as HTMLElement).getAttribute('data-count') || '1', 10),
				}),
			},
		]
	},
