...
export function useInvitations(): UseInvitationsReturn {
	...
	const acceptInvitation = useCallback(async (userWorkspaceId: string) => {
		try {
			setLoading(true)
			setError(null)
			await invitationService.acceptInvitation(userWorkspaceId)
			// Remove accepted invitation from list
			setInvitations((prev) => prev.filter((inv) => inv.userWorkspaceId !== userWorkspaceId))
	...
	const declineInvitation = useCallback(async (userWorkspaceId: string) => {
		try {
			setLoading(true)
			setError(null)
