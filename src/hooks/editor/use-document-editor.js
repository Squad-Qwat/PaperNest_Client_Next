...
export function useDocumentEditor({
  ...
  const setDefaultEditorStyles = useCallback((editor) => {
    ...
    try {
      if (editor.isEmpty) {
        ...
        editor.view.dispatch(tr)
        console.log('Default font styles set:', { fontFamily: defaultFontFamily, fontSize: defaultFontSize })
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (enableLiveblocks && yDoc) {
      console.log('🤝 Collaboration mode active - skipping Firestore content load, letting Yjs handle synchronization')
      return
    }
  ...
  const saveCurrentContent = useCallback(async (documentId) => {
