 ...
 */
export function useYjsCollaboration({ enabled = false, user = null, onSync = () => {} } = {}) {
	...
	const loadInitialContentFromFirestore = useCallback(async (yjsDoc, documentId) => {
		...
		try {
			...
			if (fragment.length > 0 || config.get('initialContentLoaded')) {
				console.log(
					'📄 Yjs document already has content or loading in progress, skipping Firestore load'
				)
				return
			}
					...
					const handleSync = (isSynced) => {
						...
						if (isSynced && !hasInitialContentLoaded.current) {
							...
							if (fragment.length === 0 && !alreadyLoaded) {
								...
								console.log(
									'📄 Yjs document already has content or initial load completed, skipping Firestore load'
								)
								hasInitialContentLoaded.current = true
							}
						}
	...
	useEffect(() => {
		...
		yProvider.awareness.setLocalStateField('user', {
			...
			color:
				liveblocksUserInfo.color ||
				getUserColor(user?.userId || user?.id || user?.uid || 'anonymous'),
	...
	const setLocalAwarenessState = useCallback(
		(state) => {
