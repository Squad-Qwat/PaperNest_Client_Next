...
export default function DocumentPage() {
  ...
  const fetchDocument = useCallback(async () => {
    ...
    try {
      ...
      try {
        const roomStateData = await documentsService.getDocumentWithRoomState(
          documentId
        )
      ...
      toast.error("Failed to load document. Redirecting to documents page.")
      router.push("/documents")
      return null
    } finally {
      setIsLoading(false)
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (response.allSucceeded) {
        ...
        const failedOpsList = failedOps
          .map((op) => `${op.operationType} failed`)
          .join(", ")
        toast.warning(`Save completed with issues: ${failedOpsList}`)
      }

      /* Removed redundant editor internal save, content is already handled by batch operations above */
    } catch (error) {
