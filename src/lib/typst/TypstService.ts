...
class TypstService {
	...
	async compileTypst(
		...
		try {
			...
			const hasErrors = diagnostics?.some((d: any) => {
				...
				return (
					sev === 'error' || sev === 'Error' || (typeof sev === 'object' && sev.Error !== undefined)
				)
			})
			...
			this.showNotification(
				'success',
				t(`Typst {format} compilation completed`, { format: format.toUpperCase() }),
				operationId,
				format,
				3000
			)
	...
	private createOutputFiles(mainFile: string, result: TypstCompileResult): FileNode[] {
		...
		if (result.pdf && result.format === 'pdf') {
			...
			const content =
				result.format === 'svg' ? result.svg : new TextDecoder().decode(result.canvas!)
	...
	private createFileNode(
		...
		const buffer =
			content instanceof ArrayBuffer ? content : (toArrayBuffer(content) as ArrayBuffer)
		return {
			id: nanoid(),
			name,
	...
	private handleCompilationError(
		...
		this.showNotification(
			'error',
			t(`Typst compilation failed: {message}`, { message }),
			operationId,
			format,
			5000
		)
	...
	private showNotification(
		...
		if (!this.areNotificationsEnabled() || format.toLowerCase().includes('canvas')) return
	...
	private parseDiagnosticsFromError(error: any): any[] {
		...
		let match = diagPattern.exec(errorStr)
		while (match !== null) {
			const hintsStr = match[4]
			const hints: string[] = []

			const hintPattern = /"([^"]+)"/g
			let hintMatch = hintPattern.exec(hintsStr)
			while (hintMatch !== null) {
				hints.push(hintMatch[1])
				hintMatch = hintPattern.exec(hintsStr)
			}
			...
			match = diagPattern.exec(errorStr)
		}
	...
	private formatDiagnostics(diagnostics?: any[]): string {
		...
		for (const diag of diagnostics) {
			...
			const severityStr = typeof severity === 'string' ? severity.toLowerCase() : 'error'
