	...
	return [
		tool(
			async ({ fromLine, toLine, full }) =>
				JSON.stringify({ action: 'read_document', fromLine, toLine, full: full ?? true }),
			{
				name: 'read_document',
				...
				schema: z.object({
					...
					full: z
						...
						.optional()
						.default(true)
						.describe('Read the entire document. Defaults to true.'),
				}),
			}
		),
		tool(
				...
				}),
			{
				...
				schema: z.object({
					...
					occurrence: z
						.number()
						.optional()
						.describe('Occurrence index for anchor match (1-based). Defaults to 1 when unique.'),
					caseSensitive: z
						.boolean()
						.optional()
