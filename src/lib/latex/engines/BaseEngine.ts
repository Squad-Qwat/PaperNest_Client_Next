...
export abstract class BaseEngine {
	...
	protected status: 'unloaded' | 'loading' | 'ready' | 'compiling' | 'error' = 'unloaded'
	...
	abstract compile(mainFileName: string, fileTree: any[]): Promise<CompileResult>
			...
			return new Promise((resolve, reject) => {
				const checkStatus = () => {
					if (this.status === 'ready') {
						...
						reject(new Error(`Failed to initialize ${this.config.name} engine`))
					} else {
