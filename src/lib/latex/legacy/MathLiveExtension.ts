...
import { MathEditWidget, MathPreviewWidget } from './mathlive/MathWidget'
...
class MathLiveProcessor {
	...
	private renderPreview(region: MathRegion, mouseX: number, mouseY: number): void {
		...
		this.outsideClickHandler = (e: MouseEvent) => {
			...
			if (
				this.currentOverlay &&
				(this.currentOverlay.contains(target) || this.isClickInMathLiveUI(target))
			) {
				return
			}
	...
	private handleSave(region: MathRegion, newContent: string): void {
		...
		const normalizedContent = shouldUnescapeAmp() ? newContent.replace(/\\&/g, '&') : newContent
