...
class AutocompleteProcessor {
	...
	private async getMergedTypstCompletions(
		...
		const filteredRefs = refLabels.filter(
			({ label }) => !partial || label.toLowerCase().includes(partial.toLowerCase())
		)

		const filteredCitations = bibCache.filter(
			(entry) =>
				!partial ||
		...
		const citationOptions = this.bibliographyHandler.createCitationOptions(
			filteredCitations,
			partial
		)
		...
		const partialStart = this.referenceHandler.getReferenceCompletionStart(
			context,
