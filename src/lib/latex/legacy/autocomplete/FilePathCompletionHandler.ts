...
export class FilePathCompletionHandler {
	...
	getCompletions(context: CompletionContext, currentFilePath: string): CompletionResult | null {
		...
		let commandInfo: { command: string; partial: string; fileTypes: string } | null = null
		...
		switch (fileTypes) {
			...
			case 'bib':
				...
				break
			case 'data':
				candidates = cache.allFiles.filter((path) => /\.(csv|json|yaml|yml|toml)$/i.test(path))
				break
			default:
		...
		const options = candidates
			...
			.filter(
				({ relativePath }) => !partial || relativePath.toLowerCase().includes(partial.toLowerCase())
			)
			.sort((a, b) => {
