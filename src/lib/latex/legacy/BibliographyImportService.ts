...
class DefaultBibTexParser implements BibTexParser {
	...
	updateEntryInContent(content: string, entry: BibEntry): string {
		...
		return content.substring(0, position.start) + newEntryContent + content.substring(position.end)
	}
}
...
export class BibliographyImportService {
				...
				const existing = this.parser
					...
					.find(
						(e) =>
							e.key === entryKey ||
							(remoteId &&
								(e.remoteId || e.fields['remote-id'] || e.fields['external-id']) === remoteId)
					)
	...
	private async updateContent(
		...
		if (file.documentId) {
			...
			if (projectId) {
				await collabService.updateDocumentContent(projectId, file.documentId, updater)
			}
		}
