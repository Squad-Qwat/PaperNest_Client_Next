...
import type { DocumentStatistics, FileStatistics, StatisticsOptions } from '../types/statistics'
	...
	private async extractIncludedFiles(
		...
		while (match !== null) {
			...
			const file = allFiles.find(
				(f) => f.path === filename || f.path === `/${filename}` || f.path.endsWith(`/${filename}`)
			)
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (line.startsWith('File: ') || line.startsWith('Included file: ')) {
				if (currentFileStat && (inFileSummary || !inVerboseOutput)) {
					if (
						currentFileStat.words > 0 ||
						currentFileStat.headers > 0 ||
						currentFileStat.captions > 0
					) {
