...
// biome-ignore lint/complexity/noStaticOnlyClass: LaTeXConverter uses static methods as a namespace
export class LaTeXConverter {
	...
	static toHTML(latex: string): string {
		...
		html = html.replace(
			/\\centering\s*/g,
			'<span data-type="latex-cmd" data-cmd="centering"></span>'
		)
		html = html.replace(/\\noindent\s*/g, '<span data-type="latex-cmd" data-cmd="noindent"></span>')
		html = html.replace(
			/\\label\{([^{}]*)\}\s*/g,
			'<span data-type="latex-cmd" data-cmd="label" data-val="$1"></span>'
		)

		// Font size commands (preserve them)
		html = html.replace(
			/\\(tiny|scriptsize|footnotesize|small|normalsize|large|Large|LARGE|huge|Huge)\b/g,
			'<span data-type="latex-fontsize" data-size="$1"></span>'
		)

		// Font style commands (preserve them)
		html = html.replace(
			/\\(bfseries|mdseries|itshape|slshape|scshape|upshape|rmfamily|sffamily|ttfamily)\b/g,
			'<span data-type="latex-fontstyle" data-style="$1"></span>'
		)

		// Color commands with parameter
		html = html.replace(
			/\\color\{([^{}]+)\}/g,
			'<span data-type="latex-color" data-color="$1"></span>'
		)
		...
		html = html.replace(
			/\\href\{([^{}]*)\}\{((?:[^{}]|\{(?:[^{}]|\{[^{}]*\})*\})*)\}/g,
			'<a href="$1" data-latex-href="true">$2</a>'
		)
		...
		html = html.replace(
			/\\caption\{((?:[^{}]|\{(?:[^{}]|\{[^{}]*\})*\})*)\}/g,
			'<p class="latex-caption" style="text-align: center; font-style: italic; margin-bottom: 0.5rem; color: #4a5568;">$1</p>'
		)
		...
		html = html.replace(/\\(vspace|hspace)\s*\{([^{}]*)\}/g, (_match, cmd, val) => {
		...
		html = html.replace(
			/\\(newline|newpage|clearpage|columnsep|titlerule)\b/g,
			'<span data-type="latex-layout" data-cmd="$1"></span>'
		)
		...
		html = html.replace(
			...
			(match: string, env: string, content: string) => {
		...
		html = html
			...
			.map((p: string) => {
				...
				if (
					...
					trimmed.startsWith('__LATEX_PROTECTED_')
				) {
		...
		twocolumnBlocks.forEach((rawLatex, index) => {
			...
			html = html.replace(
				placeholder,
				`<div data-type="latex-protected" data-latex="${escaped}" data-block-type="twocolumn" contenteditable="false" class="latex-protected-block">[twocolumn]</div>`
			)
		})
		...
		tableBlocks.forEach((rawLatex, index) => {
			...
			html = html.replace(
				placeholder,
				`<div data-type="latex-protected" data-latex="${escaped}" data-block-type="table" contenteditable="false" class="latex-protected-block">[table]</div>`
			)
		})
		...
		figureBlocks.forEach((rawLatex, index) => {
			...
			html = html.replace(
				placeholder,
				`<div data-type="latex-protected" data-latex="${escaped}" data-block-type="figure" contenteditable="false" class="latex-protected-block">[figure]</div>`
			)
		})
		...
		bibBlocks.forEach((rawLatex, index) => {
			...
			html = html.replace(
				placeholder,
				`<div data-type="latex-protected" data-latex="${escaped}" data-block-type="thebibliography" contenteditable="false" class="latex-protected-block">[bibliography]</div>`
			)
		})
		...
		abstractBlocks.forEach((rawLatex, index) => {
			...
			html = html.replace(
				placeholder,
				`<div data-type="latex-protected" data-latex="${escaped}" data-block-type="abstract" contenteditable="false" class="latex-protected-block">[abstract]</div>`
			)
		})
	...
	static toLaTeX(html: string): string {
		...
		const listToLatex = (content: string, environment: 'itemize' | 'enumerate') => {
			...
			const items = matches
				.map((entry) => {
					const rawText = entry[1].replace(/<br\s*\/?>/g, '\n').replace(/<[^>]*>/g, '')
		...
		latex = latex.replace(
			/<span data-type="math"[^>]*data-latex="([^"]*)"[^>]*>.*?<\/span>/g,
			(_match, content) => {
		...
		latex = latex.replace(
			/<span data-type="latex-comment"[^>]*data-comment="([^"]*)"[^>]*>[^<]*<\/span>/g,
			(_match, comment) => {
		...
		latex = latex.replace(
			/<span data-type="latex-fontsize"[^>]*data-size="([^"]*)"[^>]*>[^<]*<\/span>/g,
			(_match, size) => {
				return `\\${size} `
			}
		)
		...
		latex = latex.replace(
			/<span data-type="latex-fontstyle"[^>]*data-style="([^"]*)"[^>]*>[^<]*<\/span>/g,
			(_match, style) => {
				return `\\${style} `
			}
		)
		...
		latex = latex.replace(
			/<span data-type="latex-color"[^>]*data-color="([^"]*)"[^>]*>[^<]*<\/span>/g,
			(_match, color) => {
				return `\\color{${color}}`
			}
		)
		...
		// Hyperlinks
		latex = latex.replace(/<a[^>]*href="([^"]*)"[^>]*>(.*?)<\/a>/g, (_match, url, text) => {
		...
		latex = latex.replace(
			/<span data-type="latex-cmd"[^>]*data-cmd="([^"]*)"(?:[^>]*data-val="([^"]*)")?[^>]*>[^<]*<\/span>/g,
			(_match, cmd, val) => {
				if (val) {
					return `\\${cmd}{${val}}`
				}
		...
		latex = latex.replace(
			/<div\b([^>]*)data-type="latex-protected"([^>]*)>[\s\S]*?<\/div>/g,
		...
		latex = latex.replace(
			/<span\b([^>]*)data-type="latex-inline-cmd"([^>]*)>[\s\S]*?<\/span>/g,
			(_match, beforeAttrs, afterAttrs) => {
				const attributes = `${beforeAttrs} ${afterAttrs}`
				const encoded = getAttr(attributes, 'data-latex')
				if (!encoded) return ''
		...
		latex = latex.replace(/<p>([\s\S]*?)<\/p>/g, (_match, content) => {
		...
		// Lists
		latex = latex.replace(/<ul>([\s\S]*?)<\/ul>/g, (_match: string, content: string) => {
			return listToLatex(content, 'itemize')
		})
		latex = latex.replace(/<ol>([\s\S]*?)<\/ol>/g, (_match: string, content: string) => {
			return listToLatex(content, 'enumerate')
		})

		// Layout commands (handle content rendered by Tiptap: [vspace: 10pt])
		latex = latex.replace(
			/<span data-type="latex-layout"[^>]*data-cmd="([^"]*)"(?:[^>]*data-val="([^"]*)")?[^>]*>[^<]*<\/span>/g,
			(_match, cmd, val) => {
				if (!val) {
					return `\\${cmd}`
				}
